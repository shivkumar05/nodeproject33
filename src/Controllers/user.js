  ...
  try {
    ...
    for (let i = 0; i < dataArr.length; i++) {
      ...
      if (dataArr[i].video_id) {
        var video = await uploadDeviceModel.findById({
          _id: dataArr[i].video_id,
        });
...
const getRoutineCount = async function (req, res) {
  try {
    ...
    // let routines = await routineModel.find({ userId });

    // let results = [];

    // for (let categoryId of [...new Set(routines.map(r => r.category))]) {
    //     let routinesByCategory = routines.filter(r => r.category === categoryId);
    //     let completedRoutines = routinesByCategory.filter(r => r.isCompleted).length;
    //     let expectedReps = routinesByCategory.reduce((acc, r) => acc + r.repetation, 0);
    //     let expectedSets = routinesByCategory.reduce((acc, r) => acc + r.sets, 0);
    //     let completedReps = routinesByCategory.filter(r => r.isCompleted).reduce((acc, r) => acc + r.repetation, 0);
    //     let completedSets = routinesByCategory.filter(r => r.isCompleted).reduce((acc, r) => acc + r.sets, 0);

    //     let categoryResult = {
    //         category: categoryId,
    //         noOfRoutines: routinesByCategory.length,
    //         completedRoutines,
    //         expected_reps: expectedReps,
    //         expected_sets: expectedSets,
    //         reps_done: completedReps,
    ...
    //     let routineDates = r.noOfRoutines > 0 ? routines.filter(x => x.category === r.category).map(x => x.dates).flat() : [];
    //     return routineDates.some(d => d.date === queryDate);
    // });
    // filteredResults.sort((a, b) => a.category - b.category);
    ...
    for (var j = 0; j < getRoutine.length; j++) {
      ...
      for (var i = 0; i < routine.dates.length; i++) {
        if (Querydate == routine.dates[i].date) {
          let categoryIds = Array.from(
            new Set(getRoutine.map((routine) => routine.category))
          );

          for (let i = 0; i < categoryIds.length; i++) {
            let routinesByCategory = getRoutine.filter(
              (routine) => routine.category === categoryIds[i]
            );
            ...
            let complete = routinesByCategory.reduce(
            ...
            let reps = routinesByCategory.reduce(
              (accumulator, currentValue) => {
            ...
            let sets = routinesByCategory.reduce(
              (accumulator, currentValue) => {
            ...
            let completedReps = routinesByCategory.filter(
              (routine) => routine.isCompleted === true
            );
            if (completedReps) {
              let repsDone = completedReps.reduce(
                (accumulator, currentValue) => {
                  return accumulator + currentValue.repetation;
                },
                0
            ...
            let completedSets = routinesByCategory.filter(
              (routine) => routine.isCompleted === true
            );
            if (completedSets) {
              let setsDone = completedSets.reduce(
                (accumulator, currentValue) => {
                  return accumulator + currentValue.sets;
                },
                0
    ...
    results.sort((a, b) => a.category - b.category);
...
const getNewRoutine = async function (req, res) {
  try {
    ...
    const drills = await routineModel
      .find({ userId: userid, isCompleted: false, $or: [filter] })
      .lean();
...
const getPastRoutine = async function (req, res) {
  try {
    ...
    let userDrill = await routineModel
    ...
    for (let i = 0; i < arr.length; i++) {
      let allDrills = await myDrillModel
        .find({ routine_id: arr[i]._id })
        .lean();
...
// console.log(myDrillsCreated, "aaaaa")
